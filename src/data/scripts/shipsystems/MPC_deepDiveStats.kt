...
class MPC_deepDiveStats: BaseShipSystemScript() {
    ...
    override fun apply(
        ...
        if (state == ShipSystemStatsScript.State.IDLE) {
            ...
            if (ship.ai != null && ship.fluxLevel >= 0.9f || ((ship.aiFlags.hasFlag(ShipwideAIFlags.AIFlags.BACKING_OFF)) || (ship.aiFlags.hasFlag(ShipwideAIFlags.AIFlags.NEEDS_HELP) || ship.aiFlags.hasFlag(ShipwideAIFlags.AIFlags.IN_CRITICAL_DPS_DANGER))) && ship.isPhased) {
                ship.giveCommand(ShipCommand.USE_SYSTEM, null, 0)
            }
        }

        else if (state == ShipSystemStatsScript.State.IN || state == ShipSystemStatsScript.State.ACTIVE || state == ShipSystemStatsScript.State.OUT) {
            val ship = stats.entity as? ShipAPI ?: return
            ...
            if (ship.fluxLevel < 0.9f && !ship.aiFlags.hasFlag(ShipwideAIFlags.AIFlags.BACKING_OFF) && !ship.aiFlags.hasFlag(ShipwideAIFlags.AIFlags.NEEDS_HELP)) {
    ...
    fun doUnapply(stats: MutableShipStatsAPI, id: String) {
        val ship = stats.entity as? ShipAPI ?: return

        if (ship.customData["MPC_didDiveExplosion"] != true) return
